...
const Feed = ({newsData}) => {
    ...
    const sixNews = newsData.slice(15,21);
    const twoNews = newsData.slice(21,23);
    const fourNews = newsData.slice(23,27);
    const otherNews = newsData.slice(27);
  ...
  return (
    <div className='feed grid grid-cols-4 gap-3 auto-rows-[146px] w-full' >
      ...
      {
        otherNews.map((newss, index) => <NewBox newss={newss} key={index} />)
      }
